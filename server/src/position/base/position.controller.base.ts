...
export class PositionControllerBase {
    ...
    if (invalidAttributes.length) {
      ...
      throw new common.ForbiddenException(
        `Updating the relationship: ${
          invalidAttributes[0]
        } of ${"Position"} is forbidden for roles: ${roles}`
      );
    }
  ...
  async findManyPlayer(
    ...
    const results = await this.service.findPlayer(params.id, {
      where: query,
      select: {
        age: true,
        birthday: true,
